...
void input_to_file(const char *filename, const char *input){
...
void save_to_intresting(const char *input, int seed_num){
    char filename[256];
    snprintf(filename, sizeof(filename), "interesting/seed_%d.txt", seed_num);
...
int run_target(const char *input) {
    
    input_to_file(INPUT_FILE, input);
    ...
    if (pid == 0)
    {
        char *args[] = {"./target", INPUT_FILE, NULL};
                ...
                if (fgets(buffer, sizeof(buffer), f)) {
                    ...
                    if (update_coverage()) {
                         if (num_seeds < MAX_SEEDS) {
                            strcpy(seed_pool[num_seeds].seed_buffer, buffer);
                            num_seeds++;
                        }
                    }
                }
    ...
    while (true)
    {
        ...
        if (update_coverage())
        {
            ...
            if (num_seeds < MAX_SEEDS)
            {
                ...
                save_to_intresting(input, num_seeds);
            }
        }
    }
